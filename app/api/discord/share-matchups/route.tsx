...
const globalForWasm = globalThis as { resvgWasmReady?: boolean; resvgWasmInit?: Promise<void> };
...
export async function POST(request: Request) {
  try {
    ...
    if (!globalForWasm.resvgWasmReady) {
      if (!globalForWasm.resvgWasmInit) {
        globalForWasm.resvgWasmInit = (async () => {
          const wasmUrl = new URL(
            "@resvg/resvg-wasm/index_bg.wasm",
            import.meta.url
          );
          const wasmData = await readFile(wasmUrl);
          await initWasm(wasmData);
          globalForWasm.resvgWasmReady = true;
        })();
      }
      await globalForWasm.resvgWasmInit;
    }
    ...
    return NextResponse.json(
      {
        error:
          err instanceof Error
            ? `Discord共有の処理に失敗しました。${err.message}`
            : "Discord共有の処理に失敗しました。",
      },
      { status: 500 }
    );
